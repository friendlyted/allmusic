...
import pro.friendlyted.ears.music.base.Accidental.*
import pro.friendlyted.ears.music.base.MidiPitch.*

enum class MusicPitch constructor(
        val octave: Octave,
        val pitch: Pitch,
        val accidental: Accidental,
        val midiPitch: MidiPitch
) {
    ...
    companion object {
        ...
        fun firstOctave(): Array<MusicPitch> {
            return arrayOf(
                    C4_FLAT,
                    C4,
                    C4_SHARP,
        ...
        fun forPitch(pitch: MidiPitch): Array<MusicPitch> {
            return values()
                .filter { it.midiPitch == pitch }
                .toTypedArray()
        }

        fun forValues(octave: Octave, pitch: Pitch, accidental: Accidental?): MusicPitch {
            val name =
                pitch.name + octave.number + if (accidental == null || accidental == NATURAL) "" else "_" + accidental.name
            return valueOf(name)
        }
    }
}
